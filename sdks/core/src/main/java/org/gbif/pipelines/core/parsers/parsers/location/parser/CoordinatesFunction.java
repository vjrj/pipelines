package org.gbif.pipelines.core.parsers.parsers.location.parser;
...
public class CoordinatesFunction {
  ...
  public static Set<String> getIssueTypes(UnaryOperator<LatLng> transformation) {
    ...
    if (transformation == NEGATED_COORDS_FN) {
      return new TreeSet<>(Arrays.asList(PRESUMED_NEGATED_LATITUDE.name(), PRESUMED_NEGATED_LONGITUDE.name()));
    }
